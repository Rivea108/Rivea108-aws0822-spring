...
public class MemberController { //객체생성
	...
	@RequestMapping(value="memberInfor.aws", method=RequestMethod.GET)
	public String memberInfor() {
		//logger.info("memberInfor들어옴");
		
		
		
		//logger.info("tt값은? :" + tt.test());
		
		return "WEB-INF/member/memberInfor";
	}	
	...
	public String memberJoinAction(MemberVo mv) {
	    ...
	    if (mv.getMemberpwd() == null || mv.getMemberpwd().isEmpty()) {
	        // 비밀번호가 비어 있으면 알림을 출력하고, 다시 회원가입 페이지로 리다이렉트
	        logger.info("비밀번호가 비어 있습니다.");
	        return "redirect:/member/memberJoin.aws"; // 비밀번호가 없으면 다시 회원가입 페이지로 리다이렉트
	    }
	    ...
	    String path = (value == 1) ? "redirect:/board/boardLogin.aws" : "redirect:/member/memberJoin.aws";
	    return path;
	}
	...
	public String memberLoginAction(
		...
		if(bCryptPasswordEncoder.matches(memberpwd, reservedPwd)) {
			...
			rttr.addAttribute("memberName", mv.getNickname());
