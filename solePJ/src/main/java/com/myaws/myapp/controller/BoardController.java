...
public class BoardController {
    ...
    @Resource(name = "uploadPath") // 이름으로 주입
    ...
    @RequestMapping(value = "boardMain.aws")
    public String boardMain(Model model) {
        // 각 채널에 대해 게시글 10개씩 가져오기
    	
    	System.out.println("boardMain 메소드 호출됨1"); //이게 안됨
    	
        List<BoardVo> Cp = boardService.getcPost(1, 10); // 채널 1 게시글
        List<BoardVo> Cp1 = boardService.getcPost(2, 10); // 채널 2 게시글
        List<BoardVo> Cp2 = boardService.getcPost(3, 10); // 채널 3 게시글
        List<BoardVo> Cp3 = boardService.getcPost(4, 10); // 채널 4 게시글

        System.out.println("boardMain 메소드 호출됨2");
        
        // 게시글 데이터를 모델에 추가
        model.addAttribute("channel1Posts", Cp);
        model.addAttribute("channel2Posts", Cp1);
        model.addAttribute("channel3Posts", Cp2);
        model.addAttribute("channel4Posts", Cp3);

        System.out.println("boardMain 메소드 호출됨3");
       
        return "WEB-INF/board/boardMain"; // boardMain.jsp로 포워딩
    }
    
    @RequestMapping("redirectBoardMain.aws")
    public String redirect() {
        System.out.println("2");
        return "WEB-INF/board/boardMain";  // 정상적인 URL 경로로 수정
    }
